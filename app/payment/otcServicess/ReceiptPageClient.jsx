...
export default function ReceiptPageClient() {
  ...
  const title = useMemo(() => {
    ...
    if (serviceType === "personal_training") return "Personal Training Receipt";
  ...
  const status = (
    membership?.status ?? groupClass?.status ?? personal?.status ?? "pending"
  ).toString();
  const paymentMethod = (
    membership?.payment_method ?? groupClass?.payment_method ?? personal?.payment_method ?? "counter"
  ).toString();
    ...
    <div className="mx-auto max-w-3xl p-4 md:p-8">
      <Card className="shadow-lg border-slate-200 print:shadow-none">
        <CardHeader className="space-y-1">
          ...
          <CardDescription>
            Thank you! Your pay-on-counter order has been recorded and is currently pending payment at the front desk.
          </CardDescription>
        </CardHeader>
        <Separator />
        ...
        <CardContent className="grid gap-6 md:grid-cols-5 print:grid-cols-5">
          <div className="md:col-span-5 space-y-4">
            <div className="rounded-2xl border border-slate-200 p-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Button size="sm" variant="outline" className="gap-1" onClick={copyRef}>
              ...
              <div className="mt-3 grid grid-cols-2 gap-3">
                ...
                <div className="rounded-xl bg-slate-50 border border-slate-200 p-3">
                  ...
                  <div className="flex items-center gap-2 text-base font-medium">
                    ...
                    {(paymentMethod === "counter" || paymentMethod === "otc") ? "Pay on the Counter" : paymentMethod}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-slate-200 p-4">
              <div className="text-sm font-semibold text-slate-700 mb-2">Order Details</div>
              {loading && <div className="text-slate-500 text-sm">Loading receipt…</div>}
              {error && <div className="text-red-600 text-sm">{error}</div>}
              {!loading && !error && (
                <div className="space-y-1">
                  {membership && (
                    <>
                      <LabelValue label="Service" value={membership.plan_title || "Membership"} />
                      <LabelValue label="Start date" value={membership.start_date ? new Date(membership.start_date).toLocaleString() : "—"} />
                      <LabelValue label="End date" value={membership.end_date ? new Date(membership.end_date).toLocaleString() : "—"} />
                      <LabelValue label="Months" value={String(membership.total_months)} />
                      <LabelValue label="Days left" value={String(membership.days_left)} />
                      <LabelValue label="Keycard" value={membership.keycard_id || "—"} />
                    </>
                  )}

                  {groupClass && (
                    <>
                      <LabelValue label="Class" value={groupClass.class_name} />
                      <LabelValue label="Start date" value={groupClass.start_date ? new Date(groupClass.start_date).toLocaleString() : "—"} />
                      <LabelValue label="End date" value={groupClass.end_date ? new Date(groupClass.end_date).toLocaleString() : "—"} />
                      <LabelValue label="Total days" value={String(groupClass.total_days)} />
                      <LabelValue label="Keycard" value={groupClass.keycard_id || "—"} />
                    </>
                  )}

                  {personal && (
                    <>
                      <LabelValue label="Training" value={`${personal.title} (${personal.training_type})`} />
                    </>
                  )}
        ...
        <CardFooter className="flex flex-col sm:flex-row gap-2 sm:justify-between">
          <div className="flex items-center gap-2 text-xs text-slate-500">
            <Check className="h-4 w-4" /> Reference: <span className="font-mono">{refId || "—"}</span>
          </div>
